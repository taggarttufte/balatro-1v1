...
Reproduction: `mp/ev/active_poc/NOTES.md` §0 has the five commands in order. Stages 1 and 4
are resumable. Run dirs are under `mp/ev/runs/active_poc/` (gitignored).

**Files created** (all new; nothing existing was modified, nothing committed):

* `mp/ev/active_poc/` — `jobs.py`, `corpus.py`, `select.py`, `training.py`, `bench.py`,
...
* `mp/results/active_poc_2026-08-25.md` (this file) and `.json` (every number above)
