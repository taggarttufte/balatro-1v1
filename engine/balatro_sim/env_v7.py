...
The SELECTING_HAND phase uses a two-level action:
  1. Intent: PLAY / DISCARD / USE_CONSUMABLE[0] / USE_CONSUMABLE[1]
     (Discrete(4), from the intent head)
  2. Card subset: selected via card scoring head -> subset distribution -> sample

For PLAY and DISCARD the subset is the cards to play or throw away. For the
consumable intents it is the consumable's CARD TARGET — until 2026-07-30 the
subset was discarded and an empty target list was sent, so every card-modifying
tarot and spectral was consumed for zero effect (see SIM_AUDIT §A1).
...
Observation: 443 dims (was 434 before the 2026-07-29 audit widened the boss
one-hot to cover all 24 real bosses).
...
from .game import (BalatroGame, State,
                   REGULAR_BOSS_BLINDS, ALL_REGULAR_BOSS_BLINDS, SHOWDOWN_BOSS_BLINDS)
...
from .card_selection import (
    ...
    INTENT_USE_CONSUMABLE_0, INTENT_USE_CONSUMABLE_1, CONSUMABLE_INTENTS,
    consumable_slot, enumerate_subsets, apply_action, HypotheticalScorer,
)
...
#   14  reroll
#   15  leave_shop
#   16  use a planet from ANY consumable slot (was: slot 0 only)
...
# Boss one-hot, derived from the game's selection pools rather than hardcoded.
# This was a fixed list of 15 keys; the 2026-07-29 audit corrected and expanded
# the pools to 19 regular + 5 showdown bosses, and a hardcoded 15 would have left
# 9 of them encoded as all-zeros — indistinguishable from "no boss".  W5: the four
# face-down bosses (House / Wheel / Mark / Fish) are drawable since W2 and modelled
# since W5, so the one-hot covers all 23 regular + 5 showdown = 28 game bosses.
BOSS_TYPES     = list(ALL_REGULAR_BOSS_BLINDS) + list(SHOWDOWN_BOSS_BLINDS)
N_BOSS_TYPES   = len(BOSS_TYPES)
SHOP_CONTEXT   = 2 + 27 + N_BOSS_TYPES + 8 + 8   # reroll, vouchers, boss, deck x2
...
OBS_DIM = (GAME_SCALARS
           ...
           + SHOP_CONTEXT)
# = 14 + 240 + 50 + 42 + 12 + 16 + (2+27+24+8+8) = 443
# NOTE: OBS_DIM changed from 434 to 443. Any pre-audit checkpoint is incompatible
# (the V7 Run 4 checkpoint was already lost, so nothing depended on 434).
...
class BalatroV7Env(gym.Env):
    ...
    def step_hand(self, intent: int, subset: tuple[int, ...]):
        ...
        if intent == INTENT_PLAY:
            ...
            # The subset the agent selected IS the consumable's card target.
            # Until 2026-07-30 this sent `target_cards: []` with a hardcoded
            # slot 0, so every card-modifying tarot/spectral was consumed for
            # zero effect and slot 1 was unreachable. See SIM_AUDIT §A1.
            slot = consumable_slot(intent)
            if slot is not None and slot < len(gs.consumable_hand):
                targets = [i for i in subset if i < len(gs.hand)]
                gs.step({"type": "use_consumable",
                         "consumable_idx": slot, "target_cards": targets})
    ...
    def step_phase(self, action: int):
        ...
        if gs.state == State.BLIND_SELECT:
            ...
            if 2 <= action <= 8:
                ...
                # Use a planet in the shop. Planets take no card target, so this
                # path stays targetless — card-targeting consumables are used
                # during SELECTING_HAND, which is where the subset head exists to
                # choose targets with.
                #
                # Previously this only looked at slot 0, so a planet sitting in
                # slot 1 behind a tarot could never be used.
                from .consumables import ALL_PLANETS as _AP
                slot = next((i for i, k in enumerate(gs.consumable_hand)
                             if k in _AP), None)
                if slot is not None:
                    gs.step({"type": "use_consumable",
                             "consumable_idx": slot, "target_cards": []})
    ...
    def _best_hand_score(self, gs) -> float:
        ...
        This is a REWARD ESTIMATE and must have zero effect on game state. Until
        2026-08-21 the expensive phase ran score_hand() on the live jokers, the
        live planet dict and `rng=gs.rng`, so every Lucky/Bloodstone/Business/
        8 Ball/Misprint/Space roll inside the hypotheticals advanced the seeded
        stream by a hand-dependent amount before the real play, scaling jokers
        were bumped once per candidate, and Space Joker levelled hands for free
        (MP_UPDATE_LIST_2026-08 §3). The scoring now goes through
        HypotheticalScorer, which uses a private RNG seeded from a snapshot of
        gs.rng and fresh copies of every mutable object — see card_selection.py.
        """
        import itertools
        hand = gs.hand
        if not hand:
        ...
        # W5: evaluate with the board's Four Fingers / Shortcut / Smeared flags, exactly
        # as BalatroGame._play_hand will.
        flags = gs.hand_eval_flags() if hasattr(gs, "hand_eval_flags") else {}
        for k in range(1, min(6, n + 1)):
            for combo in itertools.combinations(range(n), k):
                cards = [hand[i] for i in combo]
                try:
                    ht, sc = evaluate_hand(cards, **flags)
                except Exception:
                    continue
        ...
        best = 0.0
        # The best-score estimate must model held-in-hand and full-deck effects
        # too (model_held=True), or it understates any Steel/Baron build and the
        # card-quality reward saturates. The scorer copies jokers, cards and
        # planet levels per candidate and never touches gs.rng.
    ...
    def get_intent_mask(self) -> np.ndarray:
        ...
        # USE_CONSUMABLE[i]: valid only if slot i actually holds a consumable
        for slot, intent_id in enumerate(CONSUMABLE_INTENTS):
    ...
    def _encode_obs(self) -> np.ndarray:
        ...
        # ── Hand cards (8 × 30 = 240) ─────────────────────────────────────
        # Precompute hand-level stats for new features.  Face-down cards (The House /
        # Wheel / Mark / Fish — W5) are hidden information: only their "present" bit
        # is set and they do not feed the other cards' suit/rank/connectivity stats.
        hand = gs.hand
        ...
        for slot in range(N_HAND_SLOTS):
            if slot < len(hand):
                ...
                if getattr(c, "face_down", False):
                    obs[idx+25] = 1.0  # present, identity unknown
                    idx += CARD_FEATURES
                    continue
