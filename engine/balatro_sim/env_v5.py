...
from .card_selection import HypotheticalScorer
from .shop import (
...
class BalatroSimEnvV5(gym.Env):
        ...
        if state == State.BLIND_SELECT:
            ...
            if action <= 19 and self._play_combos:
                ...
                if game.state == State.ROUND_EVAL:
                    # W2: the engine stops at ROUND_EVAL after a cleared blind; the cash-out
                    # (payout, tags, next-ante draws, shop) runs on the next step. Skipping
                    # no longer opens a shop, so this is the only way into the shop phase.
    ...
    def _step_pack_open(self, action: int) -> float:
        ...
        from .shop import BoosterChoice as _BC

        if isinstance(chosen, _BC):
            # W2 pack entries (W5 fix): the pick leaves the pack list (so _exit_pack_substate
            # releases only the unpicked cards); playing cards / jokers / slot-bound
            # consumables go through the engine's own grant (run_state.acquire, Negative
            # slot bookkeeping); a targeted Tarot keeps the PACK_TARGET substate and is
            # used straight from the pack (created then removed -- nothing to acquire).
            self._pack_choices.pop(action)
            if chosen.is_consumable and chosen.key in ALL_TAROTS and _tarot_needs_target(chosen.key):
                self._pending_tarot = chosen.key
                self._shop_substate = SUBSTATE_PACK_TARGET
                self._pack_picks_left -= 1
                if self._pack_picks_left <= 0:
                    self._pack_choices = []
                return 0.0
            if chosen.is_consumable and not game._can_grant_choice(chosen):
                # no free slot: the legacy env used the card immediately
                if chosen.key in ALL_TAROTS:
                    apply_tarot(game, chosen.key)
                elif chosen.key in ALL_PLANETS:
                    apply_planet(game, chosen.key)
            else:
                game._grant_choice(chosen)
            self._pack_picks_left -= 1
    ...
    def _exit_pack_substate(self):
        # W5: the unpicked pack cards are Card:remove'd (run_state.release_pack) so
        # `used_jokers` stays faithful -- the env took the choices off the game earlier.
        from .shop import BoosterChoice as _BC
        gens = [c.gen for c in self._pack_choices if isinstance(c, _BC) and c.gen is not None]
        if gens:
            self.game.run_state.release_pack(gens)
        self._shop_substate = SUBSTATE_NORMAL
        self._pack_choices  = []
        self._pack_picks_left = 0
    ...
    def _update_play_combos(self):
        ...
        # Side-effect-free scorer: until 2026-08-21 this loop called score_hand()
        # with no rng= (so stochastic jokers fell through to the unseeded global
        # `random`) on the live jokers / planet dict, mutating both and making
        # the combo ranking non-reproducible. See card_selection.HypotheticalScorer.
        # model_held=False keeps the historical ranking semantics (no held-card /
        # full-deck modelling) — only the isolation changed.
        scorer = HypotheticalScorer(gs, model_held=False)
        flags = gs.hand_eval_flags() if hasattr(gs, "hand_eval_flags") else {}   # W5
        scored = []
        for size in range(1, min(n, 5) + 1):
            for combo in _combinations(list(range(n)), size):
                cards = [hand[i] for i in combo]
                try:
                    hand_type, scoring_cards = _eval_hand(cards, **flags)
                    priority = _HP.index(hand_type) if hand_type in _HP else 99
                    actual_score = scorer.score(cards, hand_type, scoring_cards)
...
_NO_TARGET_TAROTS = {
    "c_high_priestess", "c_emperor", "c_hermit", "c_wheel_of_fortune",
