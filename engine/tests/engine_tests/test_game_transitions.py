    ...
    def test_blind_chips_match_table(self, game):
        ...
        for ante in range(1, 9):
            for blind_idx in range(3):
                ...
                if blind_idx == 2:   # W2: the boss is the game's draw; Wall/Needle/Vessel scale it
                    from balatro_sim.game import BOSS_CHIP_MULT
                    expected = int(expected * BOSS_CHIP_MULT.get(game.current_blind.boss_key, 1.0))
...
class TestSkipBlind:
    """Real game (button_callbacks.lua:2740-2782): skipping a blind grants that blind's TAG
    (drawn at run start / Cash Out, shown on the blind-select screen) and moves straight to
    the next blind -- there is no shop and no flat $5 (the pre-W2 sim paid +$5 and opened
    a shop)."""

    def test_skip_small_grants_the_small_tag(self, game):
        tag = game.blind_tags["Small"]
        from balatro_sim.tags import TAG_DEFS, Trigger
        prev = game.dollars
        game.step({"type": "skip_blind"})
        assert game.skips == 1
        consumed_now = TAG_DEFS[tag].trigger in (Trigger.IMMEDIATE, Trigger.NEW_BLIND_CHOICE)
        if consumed_now:
            assert tag not in game.tag_state.keys() or tag == "tag_double"
        else:
            assert game.tag_state.keys() == [tag]
        if tag == "tag_skip":
            assert game.dollars == prev + 5     # Skip Tag: $5 x skips (counts this skip)
        elif tag == "tag_economy":
            assert game.dollars == prev + min(40, max(0, prev))

    def test_skip_does_not_enter_shop(self, game):
        game.step({"type": "skip_blind"})
        assert game.state in (State.BLIND_SELECT, State.BOOSTER_OPEN)   # BOOSTER_OPEN: pack tag
        assert game.blind_idx == 1
        assert game.current_blind.kind == "Big"
    ...
    def test_skip_big_moves_to_boss(self, game):
        game.blind_idx = 1
        game._prepare_next_blind()
        game.step({"type": "skip_blind"})
        if game.state == State.BOOSTER_OPEN:
            game.step({"type": "skip_booster"})
        assert game.state == State.BLIND_SELECT
        assert game.current_blind.kind == "Boss"
        assert game.current_blind.boss_key == game.boss_blind
...
class TestRoundPayouts:
    def test_hands_left_payout(self, game):
        ...
        blind_money = game.current_blind.money_reward
        game._end_round()
        earnings = hands_remaining * HAND_PAYOUT
        interest = min(prev_dollars // INTEREST_RATE, INTEREST_CAP)
        expected = prev_dollars + blind_money + earnings + interest
        assert game.dollars == expected

    def test_blind_reward_by_kind(self, game):
        """Base blind reward: Small $3, Big $4, Boss $5, Showdown $8.
        Regression for the 2026-07-29 audit — this payout was missing entirely,
        leaving the agent at ~40% of real income."""
        from balatro_sim.game import BlindInfo
        assert BlindInfo("s", "Small", 300).money_reward == 3
        assert BlindInfo("b", "Big", 450).money_reward == 4
        assert BlindInfo("x", "Boss", 600, is_boss=True).money_reward == 5
        assert BlindInfo("f", "Boss", 600, is_boss=True,
                        is_showdown=True).money_reward == 8

    def test_blind_reward_actually_paid(self, game):
        """$3 for a Small Blind must land in the bank, isolated from other income."""
        game.step({"type": "play_blind"})
        game.chips_scored = game.current_blind.chips_target
        game.state = State.ROUND_EVAL
        game.hands_left = 0      # no hand payout
        game.dollars = 0         # no interest
        game._end_round()
        assert game.dollars == 3
    ...
    def test_gold_seal_does_not_pay_for_being_held(self, game):
        """Gold SEAL pays on play, not on hold — see tests/test_seals.py.

        This previously asserted the opposite (>= prev + 3 after _end_round with
        a gold-sealed card merely held). After the blind reward landed the
        assertion passed for the wrong reason, so it is now exact and inverted.
        """
        game.step({"type": "play_blind"})
        game.hand[0].seal = "Gold"
        game.chips_scored = game.current_blind.chips_target
        game.state = State.ROUND_EVAL
        game.hands_left = 0        # isolate: no hand payout
        game.dollars = 0           # isolate: no interest
        game._end_round()
        assert game.dollars == game.current_blind.money_reward

    def test_gold_enhancement_payout(self, game):
        """Gold ENHANCEMENT is correctly $3 for being held at end of round."""
        game.step({"type": "play_blind"})
        ...
        game.dollars = 0
        game._end_round()
        assert game.dollars == game.current_blind.money_reward + 3
...
class TestConsumableUsage:
    def test_use_planet_in_selecting_hand(self, game):
        game.step({"type": "play_blind"})
        game.consumable_hand = ["c_jupiter"]
        ...
        assert "c_jupiter" not in game.consumable_hand
