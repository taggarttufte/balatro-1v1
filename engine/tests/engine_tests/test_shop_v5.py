    ...
    for seed in [42, 123, 7, 999, 0]:
        ...
        for _ in range(2000):
            ...
            if agent == "shop" and info["shop_substate"] == SUBSTATE_NORMAL:
                ...
                return obs, info
            if agent == "play":
                # W2: skipping no longer opens a shop (real game) -- play the blind and
                # clear it without scoring (debug_win_blind) to reach the shop quickly
                if env_try.game.state == State.SELECTING_HAND:
                    env_try.game.debug_win_blind()
                    env_try.game.step({"type": "advance"})
                    obs, info = env_try._get_obs_and_info()
                    continue
...
def _game_in_shop(seed=42) -> BalatroGame:
    ...
    g.reset()
    # Clear the small blind without scoring (W2: skipping no longer opens a shop)
    ...
    def test_agent_alternates_play_shop(self):
        ...
        for seed in range(20):
            ...
            for _ in range(1000):
                ...
                valid = np.where(mask)[0]
                # W2: a skip no longer opens a shop, so a uniformly random agent rarely
                # clears a blind; play the best combo when available, otherwise random
                if agent == "play" and e.game.state == State.SELECTING_HAND and mask[0]:
                    action = 0
                else:
...
class TestShopActionMask:
    ...
    def test_buy_consumable_invalid_when_slots_full(self, env):
        ...
        while len(game.consumable_hand) < game.consumable_slots:
            game.consumable_hand.append("c_mercury")
        # Ensure shop has a planet item
        game.current_shop = [ShopItem("planet", "c_mercury", "Mercury", 3)]
        mask = env.get_shop_action_mask()
        assert mask[2] is np.False_
    ...
    def test_use_consumable_valid_with_planet(self, env):
        ...
        _advance_to_shop(env)
        env.game.consumable_hand = ["c_mercury"]
        mask = env.get_shop_action_mask()
...
class TestBuyingConsumables:
    ...
    def test_buy_planet_adds_to_consumable_hand(self):
        ...
        game.dollars = 100
        game.consumable_hand = []
        item = ShopItem("planet", "c_mercury", "Mercury", 3)
        result = buy_item(game, item)
        assert result is True
        assert "c_mercury" in game.consumable_hand
    ...
    def test_buy_consumable_rejected_when_full(self):
        ...
        game.consumable_hand = ["c_mercury", "c_venus"]
        item = ShopItem("planet", "c_saturn", "Saturn", 3)
        result = buy_item(game, item)
        assert result is False
        assert "c_saturn" not in game.consumable_hand

    def test_use_planet_via_env_action(self, env):
        """Using a planet via action 15/16 should trigger apply_planet."""
        _advance_to_shop(env)
        game = env.game
        game.consumable_hand = ["c_mercury"]  # upgrades Pair
        ...
        assert "c_mercury" not in game.consumable_hand
