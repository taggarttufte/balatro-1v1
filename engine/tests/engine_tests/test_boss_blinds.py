...
class TestBossBlindStart:
    ...
    def test_goad_debuffs_spades(self, game):
        """The Goad debuffs SPADES. It debuffed Clubs before the 2026-07-29
        audit — that is The Club's effect, and The Club did not exist."""
        _setup_boss(game, "bl_goad")
        spades = [c for c in game.full_deck if c.suit == "Spades"]
        assert spades and all(c.debuffed for c in spades)
        others = [c for c in game.full_deck if c.suit != "Spades"]
        assert all(not c.debuffed for c in others)

    def test_club_debuffs_clubs(self, game):
        _setup_boss(game, "bl_club")
        clubs = [c for c in game.full_deck if c.suit == "Clubs"]
        assert clubs and all(c.debuffed for c in clubs)
        others = [c for c in game.full_deck if c.suit != "Clubs"]
        assert all(not c.debuffed for c in others)
...
class TestBossBlindPool:
    ...
    def test_boss_key_from_pool(self, game):
        from balatro_sim.game import ALL_REGULAR_BOSS_BLINDS
        game.blind_idx = 2
        game._prepare_next_blind()
        # W2: the boss comes from generate.next_boss (the game's pool incl. the
        # face-down bosses whose effect is still unmodelled)
        assert game.current_blind.boss_key in ALL_REGULAR_BOSS_BLINDS
