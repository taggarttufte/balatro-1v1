...
class TestObservation:
    ...
    def test_obs_dim_matches_its_components(self):
        """Derived rather than hardcoded. Was `== 434`; the 2026-07-29 audit grew
        the boss one-hot from a hardcoded 15 keys to the 24 real bosses, taking
        OBS_DIM to 443. Asserting the composition catches accidental drift without
        pinning a magic number."""
        from balatro_sim.env_v7 import (
            GAME_SCALARS, CARD_FEATURES, N_HAND_SLOTS, JOKER_FEATURES,
            N_JOKER_SLOTS, SHOP_FEATURES, N_SHOP_SLOTS, PLANET_FEATURES,
            CONS_FEATURES, N_CONS_SLOTS, SHOP_CONTEXT, N_BOSS_TYPES,
        )
        assert OBS_DIM == (GAME_SCALARS
                           ...
                           + SHOP_CONTEXT)
        assert SHOP_CONTEXT == 2 + 27 + N_BOSS_TYPES + 8 + 8
        # W5: the four face-down bosses are drawable (W2) and modelled (face_down) -> 28
