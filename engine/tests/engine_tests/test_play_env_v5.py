...
class TestPlayHandLogic:
    ...
    def test_use_planet_during_play(self, env):
        ...
        assert "c_mercury" not in env.game.consumable_hand
...
class TestBlindSelect:
    ...
    def test_skip_blind_grants_tag(self, env):
        """W2: skipping grants the blind's tag (no flat $5, no shop)."""
        env.reset()
        env.step(31)  # skip_blind
        assert env.game.skips == 1

    def test_skip_blind_advances(self, env):
        env.reset()
        prev_blind_idx = env.game.blind_idx
        env.step(31)  # skip
        # Real game: straight to the next blind's select screen (a pack tag may interrupt)
        assert env.game.state in (State.BLIND_SELECT, State.BOOSTER_OPEN)
        assert env.game.blind_idx == prev_blind_idx + 1
