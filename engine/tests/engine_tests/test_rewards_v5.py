...
def _advance_to_shop(env):
    ...
    for _ in range(500):
        ...
        if agent == "shop" and info["shop_substate"] == SUBSTATE_NORMAL:
            return obs, info
        if agent == "play":
            # W2: skipping no longer opens a shop -- clear the blind without scoring
            if env.game.state == State.SELECTING_HAND:
                env.game.debug_win_blind()
                env.game.step({"type": "advance"})
...
class TestQualityBaseline:
    def test_skip_blind_stays_in_play_phase(self, env):
        """W2: skipping a blind grants its tag and moves to the next blind -- no shop
        (real game), so the quality baseline is NOT initialised by a skip."""
        obs, info = env.reset()
        # Action 31 = skip blind (only valid for non-boss)
        assert env.game.blind_idx != 2
        env.step(31)  # skip blind
        assert env.game.state in (State.BLIND_SELECT, State.BOOSTER_OPEN)
        assert env.game.blind_idx == 1
