...
from balatro_sim.game_keys import gen as _gen, TAROT_KEYS, SPECTRAL_KEYS, JOKER_KEYS

TAROTS, SPECTRALS, JOKERS = set(TAROT_KEYS), set(SPECTRAL_KEYS), set(JOKER_KEYS)


def _run_state(seed="TEST1"):
    """A fresh keyed run (generate.RunState + bit-exact PseudoRandom) — W3: every
    effect roll and every created card comes from here, never from `random`."""
    return _gen.RunState(seed=seed)
...
def _make_ctx(
    ...
    **extra,
    ...
    ac = all_cards or sc
    jk = jokers or []
    rs = run_state if run_state is not None else _run_state()
    ...
    return ScoreContext(
        ...
        planet_levels=pl,
        prng=rs.rng, run_state=rs, probabilities_normal=rs.probabilities_normal,
        **extra,
    )
...
class TestSuitMultJokers:
    ...
    def test_suit_mult_no_trigger_wrong_suit(self, joker_key, suit, wrong_suit):
        ...
        # With wrong suit, j_greedy_joker shouldn't add mult
...
class TestChipJokers:
    ...
    def test_vagabond_creates_tarot_when_broke(self):
        ...
        # With $4 or less, should create a Tarot drawn through run_state ('Tarotvag1')
        ...
        assert ctx2.pending_consumables[0] in TAROTS
        assert "Tarotvag1" in ctx2.prng.keys()
...
class TestHandEvalFlags:
    ...
    def test_four_fingers_sets_flag(self):
        j = JokerInstance("j_four_fingers")
        ctx = _make_ctx(jokers=[j])
        effect = JOKER_REGISTRY["j_four_fingers"]
        effect.pre_score(j, ctx)
        assert ctx.four_finger_mode is True

    def test_smeared_joker_sets_flag(self):
        j = JokerInstance("j_smeared")
        ctx = _make_ctx(jokers=[j])
        effect = JOKER_REGISTRY["j_smeared"]
        effect.pre_score(j, ctx)
...
class TestConsumableCreatingJokers:
    def test_seance_straight_flush_creates_spectral(self):
        ...
        effect.on_hand_scored(j, ctx)
        assert len(ctx.pending_consumables) == 1 and ctx.pending_consumables[0] in SPECTRALS
        assert "Spectralsea1" in ctx.prng.keys()
    ...
    def test_superposition_ace_straight(self):
        ...
        effect.on_hand_scored(j, ctx)
        assert len(ctx.pending_consumables) == 1 and ctx.pending_consumables[0] in TAROTS
        assert "Tarotsup1" in ctx.prng.keys()
    ...
    def test_riff_raff_creates_2_jokers(self):
        ...
        # two Common jokers drawn from 'Joker1rif1' (with resamples), queued as instances
        assert len(ctx.pending_jokers) == 2
        assert all(p.key in JOKERS for p in ctx.pending_jokers)
        assert "Joker1rif1" in ctx.prng.keys()
    ...
    def test_cartomancer_creates_tarot(self):
        ...
        effect.on_blind_selected(j, ctx)
        assert len(ctx.pending_consumables) == 1 and ctx.pending_consumables[0] in TAROTS
        assert "Tarotcar1" in ctx.prng.keys()
    ...
    def test_sixth_sense_single_6(self):
        ...
        effect.on_hand_scored(j, ctx)
        assert len(ctx.pending_consumables) == 1 and ctx.pending_consumables[0] in SPECTRALS
        assert ctx.pending_destroy == cards          # the 6 is destroyed
        assert "Spectralsixth1" in ctx.prng.keys()

    def test_sixth_sense_only_first_hand_of_round(self):
        cards = [Card(6, "Hearts")]
        j = JokerInstance("j_sixth_sense")
        ctx = _make_ctx(scoring_cards=cards, hand_type="High Card", jokers=[j], hands_played=1)
        effect = JOKER_REGISTRY["j_sixth_sense"]
        effect.on_hand_scored(j, ctx)
        assert ctx.pending_consumables == [] and ctx.pending_destroy == []
...
class TestMiscJokers:
    def test_trading_card_first_discard_earns(self):
        j = JokerInstance("j_trading")
        cards = [Card(5, "Hearts")]
        effect = JOKER_REGISTRY["j_trading"]
        ctx = _make_ctx(jokers=[j])
        effect.on_discard(j, cards, ctx)
        assert ctx.pending_money == 3
        assert ctx.pending_destroy == cards          # the single discarded card is destroyed
    ...
    def test_trading_card_only_once_per_round(self):
        ...
        assert ctx2.pending_money == 0 and ctx2.pending_destroy == []
    ...
    def test_to_do_list_earns_on_match(self):
        j = JokerInstance("j_todo_list")
        j.state["target"] = "Pair"
        cards = [Card(10, "Hearts"), Card(10, "Spades")]
        ctx = _make_ctx(scoring_cards=cards, hand_type="Pair", jokers=[j])
        effect = JOKER_REGISTRY["j_todo_list"]
        effect.on_hand_scored(j, ctx)
...
class TestRegistryCompleteness:
    ...
    def test_all_registered_jokers_have_at_least_one_hook(self):
        ...
        # Passive jokers — applied by game.py / shop.py / scoring.py through
        # base.passive_modifiers, sync_probabilities, run_state.showman or the
        # held-card retrigger loop, not through a hook on the effect object.
        ...
        hooks = [
            ...
            "on_blind_skipped", "on_held_card", "on_booster_opened",
            "on_boss_ability_triggered", "on_joker_destroyed", "on_init",
            "on_reroll", "on_booster_skipped", "on_card_sold",
            "on_first_hand_drawn",   # W5: Certificate (card.lua:2462 first_hand_drawn)
        ]
