...
class TestCardEditions:
    ...
    def test_polychrome_applies_when_the_card_scores(self):
        # Ace with Polychrome, plus j_joker (+4 Mult).
        # Sequential order (audit C4): the card's x1.5 applies when the CARD
        # scores, before the joker's +4 is added.
        #   mult 1 -> x1.5 = 1.5 -> +4 = 5.5;  chips 5+11 = 16  ->  88
        # This asserted 120 previously, which pooled the +4 ahead of the x1.5 —
        # the best-case ordering the old batched engine always granted.
        assert sc([Card(rank=14, suit="Spades", edition="Polychrome")], ["j_joker"]) == 88
...
class TestScalingJokers:
    ...
    def test_j_castle_accumulates(self):
        ...
        j.state["chips"] = 0
        # Discard a Heart while the round's Castle suit (G.GAME.current_round.castle_card,
        # a game-level value) is Hearts: +3 chips
        from balatro_sim.jokers.base import JOKER_REGISTRY, ScoreContext
        effect = JOKER_REGISTRY.get("j_castle")
        ctx = ScoreContext(jokers=[j], round_cards={"castle": "Hearts"})
        effect.on_discard(j, [Card(rank=5, suit="Hearts")], ctx)
        assert j.state.get("chips", 0) == 3
        effect.on_discard(j, [Card(rank=5, suit="Spades")], ctx)
        assert j.state.get("chips", 0) == 3
...
class TestMultiJokerInteractions:
    ...
    def test_glass_plus_joker_mult(self):
        # Glass x2 applies when the card scores; j_joker's +4 comes after.
        #   mult 1 -> x2 = 2 -> +4 = 6;  chips 16  ->  96
        # Previously 160, which pooled the +4 before the x2.
        assert sc(
    ...
    def test_joker_poly_edition_multiplicative(self):
        ...
        # Jokers fire left to right: the Polychrome joker's +4 lands, then its own
        # x1.5 applies "directly after the Joker is reached", then the plain
        # joker's +4 is added on top of the multiplied value:
        #   mult 1 -> +4 = 5 -> x1.5 = 7.5 -> +4 = 11.5;  chips 16  ->  184
        # Previously 216, i.e. (1+4+4)*1.5 — both +4s pooled before the x1.5.
        assert result == 184
