"""
Test bootstrap for the agent fork.

Puts agent and engine at the front of sys.path so that

    import mcts, train        -> agent/{mcts,train}
    import balatro_sim        -> engine/balatro_sim   (the FORK, never the BRL
...
def _assert_expected(module_name: str, expected: Path) -> None:
    ...
    if got != expected:
        raise RuntimeError(
            f"agent tests imported the wrong {module_name}:\n"
            f"  got:      {got}\n"
            f"  expected: {expected}\n"
            "Something earlier on sys.path (or an already-imported module) is shadowing "
            "the fork. Run with `python -m pytest agent/tests` from the repo root, or "
            "`pytest` from inside agent."
        )
